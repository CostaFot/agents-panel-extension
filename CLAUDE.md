...
- `Data/UsageRepository.cs` — **the coordinator the UI depends on**: owns one
  ...
  `ObserveOn(TaskPoolScheduler.Default)`** hop.
...
- Surfaces: `Pages/UsagePage.cs` (hub; also the top-level command) and `Pages/UsageDockPage.cs` (dock
  band; each `GetItems()` row = one dock button, title budget ~15 chars). Both implement the explicit
  `INotifyItemsChanged` pattern: subscribe `ObserveUsage()` in the event's `add` accessor, dispose-all
  in `remove`, `List<IDisposable>` (host may add twice), `private new void RaiseItemsChanged`.
